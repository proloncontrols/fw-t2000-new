...
CMeterValue::CMeterValue(uint8_t newPrecision, uint8_t newSpacingRatio, const TypedText& newTextType, uint8_t newColorR, uint8_t newColorG, uint8_t newColorB)
{
	...
	for(uint8_t i = 0; i < precision; i++)
	{
		...
		digits[i]->widget.setColor(touchgfx::Color::getColorFromRGB(colorR, colorG, colorB));
		add(digits[i]->widget);
	}
...
void CMeterValue::setBackgroundColor(uint8_t colorRed, uint8_t colorGreen, uint8_t colorBlue)
{
