...
//  D E F I N E S
//-----------------------------------------------------------------------------
#define BASELINE_OFFSET_PERCENT   25   //Percentage of font height
...
void CMeter::display(double newValue)
{
	...
	setWidth(newWidth + leftMostCharacterLeftWidth);

	dspPtr = &value[METER_VALUE_MAX_CHAR - 1];
	for(int i = 0; i < strLength; i++)
	{
		newWidth -= dspPtr->getWidget().getWidth();
		dspPtr->getWidget().setXY(newWidth, getHeight() - dspPtr->getFont()->getFontHeight());
